...
if(isset($_POST['dateRange1'])){
    ...
    $query = "SELECT * FROM overnight WHERE enddate >= '$dateRange1' AND enddate <= '$dateRange2'";
    $result=mysqli_query($link, $query);
    $rec_count = mysqli_num_rows($result);
}else{
    $date1 = date("Y-m-d");
    $date2 = date("Y-m-d");
    $query = "SELECT * FROM overnight WHERE enddate >= '$date1' AND enddate <= '$date2'";
    $result=mysqli_query($link, $query);
    $rec_count = mysqli_num_rows($result);
...
<div id="homeContainer">
    <div class="customerListContainer">
        <h3 class="floatLeft">DEPARTURES</h3>
        <div class="clear"></div>
        <hr>
        <form id="custSortFormArrival" class="floatLeft" method="get" action="departures.php">
        ...
        <form id='arrivalDateRangeForm' class="floatLeft" method='post' action="departures.php">
            ...
            <div class='floatRight to'> to </div>
            <input type="date" id='dateRange2' onchange="$('#departureDateRangeForm').submit();" class='floatLeft' name='dateRange2' value=<?php if (isset($_POST["dateRange2"])) { echo "'$dateRange2'"; }else{ echo "'".date("Y-m-d")."'";} ?>>
            ...
            <?php
                if (empty($dateRange1) || empty($dateRange2)) {
                  ...
                  //first search for arrival dates//
                    $query = "SELECT * FROM overnight WHERE enddate >= '$dateRange1' AND enddate <= '$dateRange2' LIMIT $offset, $rec_limit";
                    $result=mysqli_query($link, $query);
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        while($row = mysqli_fetch_array($result, MYSQL_ASSOC)) {
                          ...
                          "<td data-href='booking?bookingid=".$bookingid."'>".$enddate."</td>".
                          "<td data-href='booking?bookingid=".$bookingid."'>$".number_format($owe,2)."</td>";
                        }
                    }else{
                        echo "<tr><td class='sorry' colspan='6'>Sorry you have no departures for this date range.</td></tr>";
                    }
                }else{
                  //first search for arrival dates//
                    $query = "SELECT * FROM overnight WHERE enddate >= '$dateRange1' AND enddate <= '$dateRange2' LIMIT $offset, $rec_limit";
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        echo "<tr><td colspan='6'>Sorry you have no departures for this date range.</td></tr>";
                        $dateRange1 ='';
                        $dateRange2 ='';
                    }
                }
