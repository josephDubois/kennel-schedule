...
if ($num_rows != 0) {
  while($row = mysqli_fetch_array($result, MYSQL_ASSOC)){
    ...
    echo "<div class='dogContainer'><span id='dogName".$count."'>".$name."</span><button id='".$dogid."' class='floatRight editDogButton'><i class='fa fa-pencil' aria-hidden='true'></i> Edit</button><form id='dateSelectDog".$count."' class='floatRight assignSelect'><select val='' id='bookingChoice".$count."' class='bookingChoice floatRight'></select></form><label class='bookingChoiceLabel mobileNoShow'>Assign: </label>".
    "<form class='editDog edit".$dogid."'>".
            ...
            "</select><br/>".
             ...
             "<button id='cancel".$dogid."' class='floatLeft cancelEditDog'>Cancel</button>"."<button onclick='bookingDeleteDog(".$dogid.",".$id.");' class='floatLeft deleteDog profileButton4'>Delete</button>".
  ...
  "<input type='hidden' id='dogCount' value='".$count."'/>".
  "<script>".
   "$('.editDogButton').click(function() {".
    ...
    "$('.cancelEditDog').click(function() {".
       "event.preventDefault();".
        "showEditDog();".
        "$('html, body').scrollTop('#bookingPet');".
    "});".
